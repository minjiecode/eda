...
#quality score at min and max
cat('Max:', max(wine$volatile.acidity), "| Quality: ",subset(wine, volatile.acidity == max(wine$volatile.acidity))$quality)
cat('Min:', min(wine$volatile.acidity), "| Quality: ",subset(wine, volatile.acidity == min(wine$volatile.acidity))$quality)
...
cat("% of zero citric acid: ", (count(subset(wine, citric.acid == 0))/count(wine))[, 1]*100, "%")
...
ggplot(data = wine, aes(x = residual.sugar))+geom_freqpoly(binwidth = 0.1) + geom_vline(xintercept = quantile(wine$residual.sugar, 0.95), colour = "hotpink") + scale_x_continuous(breaks = seq(0,16,1))
#quality score at min and max
cat('Max:', max(wine$residual.sugar), "| Quality: ",subset(wine, residual.sugar == max(wine$residual.sugar))$quality)
cat('Min:', min(wine$residual.sugar), "| Quality: ",subset(wine, residual.sugar == min(wine$residual.sugar))$quality)
```
Note: 
...
ggplot(wine, aes(chlorides)) +
  geom_freqpoly(binwidth = 0.005)  + geom_vline(xintercept = quantile(wine$chlorides, 0.97), colour = "hotpink") + scale_x_continuous(breaks = seq(0, 0.6,0.05))
#quality score at min and max                                                  
cat('Max:', max(wine$chlorides), "| Quality: ",subset(wine, chlorides == max(wine$chlorides))$quality)
cat('Min:', min(wine$chlorides), "| Quality: ",subset(wine, chlorides == min(wine$chlorides))$quality)
```
...
#quality score at min and max 
cat('Max:', max(wine$free.sulfur.dioxide), "| Quality: ",subset(wine, free.sulfur.dioxide == max(wine$free.sulfur.dioxide))$quality)
cat('Min:', min(wine$free.sulfur.dioxide), "| Quality: ",subset(wine, free.sulfur.dioxide == min(wine$free.sulfur.dioxide))$quality)
```
...
grid.arrange(plot1, plot2, ncol = 2)
#quality score at min and max 
cat('Max:', max(wine$total.sulfur.dioxide), "| Quality: ",subset(wine, total.sulfur.dioxide == max(wine$total.sulfur.dioxide))$quality)
cat('Min:', min(wine$total.sulfur.dioxide), "| Quality: ",subset(wine, total.sulfur.dioxide == min(wine$total.sulfur.dioxide))$quality)
```
...
#quality score at min and max 
cat('Max:', max(wine$density), "| Quality: ",subset(wine, density == max(wine$density))$quality)
cat('Min:', min(wine$density), "| Quality: ",subset(wine, density == min(wine$density))$quality)
```
...
summary(wine$pH)
#plot
ggplot(data = wine, aes(x = pH))+geom_freqpoly(binwidth = 0.01) + scale_x_continuous(breaks = seq(2.7, 4.5,0.1))
#quality score at min and max
cat('Max:', max(wine$pH), "| Quality: ",subset(wine, pH == max(wine$pH))$quality)
cat('Min:', min(wine$pH), "| Quality: ",subset(wine, pH == min(wine$pH))$quality)
```
...
cat('Max:', max(wine$sulphates), "| Quality: ",subset(wine, sulphates == max(wine$sulphates))$quality)
cat('Min:', min(wine$sulphates), "| Quality: ",subset(wine, sulphates == min(wine$sulphates))$quality)
```
...
ggplot(data = wine, aes(x = alcohol), bins = 50) + 
  ...
  scale_x_continuous(breaks = seq(8,16,1))
#max/min quality score
cat('Max:', max(wine$alcohol), "| Quality: ",subset(wine, alcohol == max(wine$alcohol))$quality)
cat('Min:', min(wine$alcohol), "| Quality: ",subset(wine, alcohol == min(wine$alcohol))$quality)
```
...
ggpairs(wine,
  ...
  upper = list(combo = wrap("box", outlier.shape = I('.'))), binwidth = 10)
```

We can see from the charts above that there are weak negative correlation between quality and volatile.acidity(-0.391) and there are moderate positive correlation between quality score and alcohol level(0.476) and weak positive correlation between quality score and citric.acid(0.226) and sulphates (0.251).

And the pH feature have strong correlation with fixed acidity (0.682) and moderate relation with citric acid(-0.542) and  ; and weak correlation withe density(-0.342), volatile.acidity (0.235) and chlorides(-0.265).


Besides, there are strong/moderate correlation between density and fixed acidity (0.668), fixed aciditiy and citric acid(0.672), volatile.acidity and residual sugar(-0.552), density and free.sulfur.dioxide, alcohol and density (-0.496).
...
Log transform: 
```{r echo = FALSE}
ggplot(wine, aes(volatile.acidity, quality))+ geom_jitter(colour= I("lightgreen"), alpha = 0.5)+ scale_x_continuous(trans = log10_trans())+ stat_smooth(method = 'lm')
```
...
ggplot(wine, aes(alcohol, ..density.., colour = factor(quality))) +
  geom_freqpoly(binwidth = 1) + scale_x_continuous(breaks = seq(0.0,1.8, 0.1))
...
```{r echo=FALSE}
#log transformation
ggplot(wine, aes(alcohol, quality.group))+ geom_jitter(color = I("dodgerblue"), alpha = 0.25) + scale_x_continuous(trans = log10_trans())
```
...
ggplot(wine, aes(citric.acid, ..density.., colour = quality.group)) +
  geom_freqpoly(binwidth = 0.1) + scale_x_continuous(breaks = seq(0.0,1.8, 0.1))
```
...
ggplot(data = wine, aes(pH, fixed.acidity))+geom_jitter(alpha = 0.25) + geom_smooth(method='lm', color = 'red') + scale_y_log10()
...
ggplot(wine, aes(pH, citric.acid))+geom_point(alpha = 0.25) + geom_smooth(method='lm', color = 'red') +  scale_x_log10()
ggplot(wine, aes(citric.acid, fill = quality.group)) +
  geom_histogram(binwidth = 0.05)
```
...
ggplot(data = wine, aes(pH, density))+ geom_jitter(alpha = 0.25) + geom_smooth(method='lm', color = 'red') + scale_y_log10()+scale_x_log10()
...
```{r echo = FALSE}
# volatile.acidity vs. sulphates vs. quality
ggplot(data = wine, aes(x = volatile.acidity, y = sulphates , colour = quality.group)) +       
 geom_point(alpha = 0.75, shape = 16)
```

```{r echo = FALSE}
# volatile.acidity vs. alcohol vs. quality
ggplot(data = wine, aes(x = volatile.acidity, y = alcohol , colour = quality.group)) +       
 geom_point(alpha = 0.75, shape = 16)
```

```{r echo = FALSE}
# volatile.acidity vs. citric.acid vs. quality
ggplot(data = wine, aes(x = volatile.acidity, y = citric.acid , colour = quality.group)) +       
 geom_point(alpha = 0.75, shape = 16)
```
...
ggplot(data = wine, aes(x =sulphates, y = alcohol , colour = quality.group)) +       
 geom_point(alpha = 0.75, shape = 16)
```

```{r echo = FALSE}
# alcohol vs. citric.acid vs. quality
ggplot(data = wine, aes(x =citric.acid, y = alcohol , colour = quality.group)) +       
 geom_point(alpha = 0.75, shape = 16)
```

```{r echo = FALSE}
# sulphates vs. citric.acid vs. quality
ggplot(data = wine, aes(x =citric.acid, y = sulphates , colour = quality.group)) +       
 geom_point(alpha = 0.75, shape = 16)
```
...
```{r echo = FALSE}
# volatile.acidity vs. fixed.acidity vs. pH
ggplot(data = wine, aes(x = volatile.acidity, y = fixed.acidity , colour = pH.group)) +       
 geom_point(alpha = 0.75, shape = 16)
```

```{r echo = FALSE}
# citric.acid vs. fixed.acidity vs. pH
ggplot(data = wine, aes(x =citric.acid, y = fixed.acidity , colour = pH.group)) +       
 geom_point(alpha = 0.75, shape = 16)
```
...
Besides, I also abserved that wine with higher level of fixed.acidity and citric.acid has lower pH level. Therefore, these two features also strengthened each other.
...
### Description One
The above boxplot shows the quality level in the x-axis and volatile.acidity in the y-axis. And we can see from the plot that the wine quality increases when the volatile acidity decreases.  
...
### Description Three
In the scatter plot above, fixed.acidity feature is shown in the y-axis and citric.acid is shown in the x-axis. And I used four different colors to represent different quailes of wine by pH level.It is easy to observe that the feature fixed.acidity and citric.acid strengthened each other, and the pH level decrease while they increase. 
...
My major interests are the relationship of chemical properties to the quality and pH level. During the analysis, I found that, dividing the quality scores into three levels (e.g. in the Plot Two above) showed a clearer trend than using the factorized group levels. In addition, I observed that quantile division is good in grouping wine variation by pH levels. 
